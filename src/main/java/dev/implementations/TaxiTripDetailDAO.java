...
import dev.interfaces.ITaxiTripDetailDAO;
 ...
 * TaxiTripDetail entities. Transaction control of the save(), update() and
 * delete() operations must be handled externally by senders of these methods or
 * must be manually added to each of these methods for data to be persisted to
 * the JPA datastore.
 * 
 * @see dev.models.TaxiTripDetail
 * @author MyEclipse Persistence Tools
 */
public class TaxiTripDetailDAO implements ITaxiTripDetailDAO {
	// property constants
	...
	public void save(TaxiTripDetail entity) {
		EntityManagerHelper.log("saving TaxiTripDetail instance", Level.INFO, null);
		try {
	 ...
	 * <pre>
	 * EntityManagerHelper.beginTransaction();
	 * TaxiTripDetailDAO.delete(entity);
	 * EntityManagerHelper.commit();
	 * entity = null;
	 * </pre>
	 * 
	...
	public void delete(TaxiTripDetail entity) {
		EntityManagerHelper.log("deleting TaxiTripDetail instance", Level.INFO, null);
		try {
			entity = getEntityManager().getReference(TaxiTripDetail.class, entity.getId());
	 ...
	 * <pre>
	 * EntityManagerHelper.beginTransaction();
	 * entity = TaxiTripDetailDAO.update(entity);
	 * EntityManagerHelper.commit();
	 * </pre>
	 * 
	 * @param entity
	...
	public TaxiTripDetail update(TaxiTripDetail entity) {
		EntityManagerHelper.log("updating TaxiTripDetail instance", Level.INFO, null);
		try {
			TaxiTripDetail result = getEntityManager().merge(entity);
	...
	public TaxiTripDetail findById(Integer id) {
		EntityManagerHelper.log("finding TaxiTripDetail instance with id: " + id, Level.INFO, null);
		try {
			TaxiTripDetail instance = getEntityManager().find(TaxiTripDetail.class, id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<TaxiTripDetail> findByProperty(String propertyName, final Object value,
			final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding TaxiTripDetail instance with property: " + propertyName + ", value: " + value,
				Level.INFO, null);
		try {
			final String queryString = "select model from TaxiTripDetail model where model." + propertyName
					+ "= :propertyValue";
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<TaxiTripDetail> findAll(final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding all TaxiTripDetail instances", Level.INFO, null);
		try {
			final String queryString = "select model from TaxiTripDetail model";
			Query query = getEntityManager().createQuery(queryString);
			if (rowStartIdxAndCount != null && rowStartIdxAndCount.length > 0) {
				int rowStartIdx = Math.max(0, rowStartIdxAndCount[0]);
				if (rowStartIdx > 0) {
